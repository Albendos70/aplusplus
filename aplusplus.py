# Fork from: https://github.com/davidcallanan/py-myopl-code
...
class IllegalCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(
            pos_start, pos_end, "A++ Input: Illegal Character?", details
        )
...
class ExpectedCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(
            pos_start, pos_end, "A++ Input: Expected Character?", details
        )
...
class InvalidSyntaxError(Error):
    def __init__(self, pos_start, pos_end, details=""):
        super().__init__(
            pos_start, pos_end, "A++ Input: Invalid Syntax?", details
        )
...
class RTError(Error):
    def __init__(self, pos_start, pos_end, details, context):
        super().__init__(
            pos_start, pos_end, "A++ Input: Runtime Error?", details
        )
...
class Parser:
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'RETURN', 'CONTINUE', 'BREAK', 'please', 'IF', 'FOR', 'WHILE', 'FUNCTION', int, float, identifier, '+', '-', '(', '[' or 'NOT'",
                )
            )
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "please"):
            res.register_advancement()
            self.advance()
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expwected 'please', 'IF', 'FOR', 'WHILE', 'FUNCTION', int, float, identifier, '+', '-', '(', '[' or 'NOT'",
                )
            )
...
class BuiltInFunction(BaseFunction):
    ...
    def no_visit_method(self, node, context):
        raise Exception(f"No execuwte_{self.name} method defined")
...
global_symbol_table.set("systemprint", BuiltInFunction.print)
global_symbol_table.set("systemprint_ret", BuiltInFunction.print_ret)
